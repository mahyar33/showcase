// **Gateway** is using an instance of **axios** for customisation such as
// credentials, content type and adding the default base url to all calls.
// Also it intercepts all calls for error handling and authentication, authorisation fallback.
...
// All requests are having `config` as an optional parameter which is **axios** [requests configs](https://github.com/axios/axios#request-config) and `session`
// can be handled if it's true, it will pass to server automatically. session gets from redux.
class Gateway {
    static #security;
